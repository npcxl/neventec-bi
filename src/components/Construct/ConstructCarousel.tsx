...
function ConstructCarousel({
  ...
  const normalizedRecords = useMemo(
    () => records.filter((item) => item.content),
    [records],
  );
  ...
  if (normalizedPictures.length === 0 && normalizedRecords.length === 0)
    return (
      <div className="flex h-full items-center justify-center rounded-xl border border-dashed border-white/10 bg-[rgba(8,23,42,0.6)] text-sm text-[#93aed0]">
        暂无数据
      </div>
  ...
  return (
    <div className="construct-carousel-shell relative h-full overflow-hidden rounded-xl border border-white/10 bg-[rgba(8,23,42,0.72)]">
      <style>{`.construct-carousel-track{animation-name:construct-carousel-scroll;animation-timing-function:linear;animation-iteration-count:infinite;animation-fill-mode:both;will-change:transform;transform:translate3d(0,0,0)}.construct-carousel-shell:hover .construct-carousel-track{animation-play-state:paused}@keyframes construct-carousel-scroll{from{transform:translate3d(0,0,0)}to{transform:translate3d(calc(-1 * var(--track-distance)),0,0)}}`}</style>
      <div className="flex h-full min-h-0 flex-col gap-2 p-3">
        <div className="text-sm font-medium text-[#dbeeff]">{title}</div>
        <div className="flex h-full min-h-0 gap-3 overflow-hidden">
          <div
            className="construct-carousel-track flex h-full w-max gap-3"
            style={{
